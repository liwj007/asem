...
public class ApplicationController {
    ...
    @RequestMapping(value = "/prize_for_check_list", method = RequestMethod.GET)
    public ResponseData getPrizeForCheck(@RequestParam(value = "token") String token,
                                               @RequestParam(value = "pageSize") Integer pageSize,
                                               @RequestParam(value = "pageNum") Integer pageNum) throws WSPException {
        User user = userService.getUserByToken(token);
        PageInfo pageInfo = applicationService.getPrizeForCheck(user,pageNum,pageSize);
        ResponseData responseData = new ResponseData();
        responseData.setSuccessData(pageInfo);
        return responseData;
    }
