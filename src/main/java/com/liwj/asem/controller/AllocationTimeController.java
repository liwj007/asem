...
public class AllocationTimeController {
    ...
    public ResponseData getSelectOfUnAllocationTime(@RequestParam(value = "token") String token,
                                                    ...
                                                    @RequestParam(value = "needGrade", required = false) Boolean needGrade) throws WSPException {
        User user = userService.getUserByToken(token);
        List<SelectOfScholarshipBO> res = prizeService.getSelectOfUnAllocationTimeScholarships(user,scholarshipType,needGrade);
    ...
    public ResponseData getScholarshipsOfAllocatedTime(@RequestParam(value = "token") String token,
        ...
        User user = userService.getUserByToken(token);
        PageInfo pageInfo = prizeService.getScholarshipsOfAllocatedTime(user, pageNum, pageSize);
        ResponseData responseData = new ResponseData();
    ...
    public ResponseData getCollegeAwardInfo(@RequestParam(value = "token") String token,
        ...
        User user = userService.getUserByToken(token);
        TimeLimitBO bo = prizeService.getDetailOfAllocatedTime(user,id);
        ResponseData responseData = new ResponseData();
